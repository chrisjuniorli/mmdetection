...
class AnchorFreeHead(nn.Module):
    ...
    def loss(self,
       ...
       # flatten_centerness = torch.cat(flatten_centerness)
        ...
        centerness_targets_sqrt = torch.sqrt(centerness_targets)
        #pos_inds = pos_inds_tem[centerness_targets]
       ...
       # pdb.set_trace()
        ...
        if self.center_flag:
            ...
            flatten_center = flatten_labels
            flatten_center = flatten_center.type_as(centerness_targets_sqrt)
            flatten_center[pos_inds_tem] = centerness_targets_sqrt
            flatten_center[flatten_center==0] = 1
            flatten_center = flatten_center.expand(20,-1)
            flatten_center = flatten_center.transpose(0,1)
            loss_cls = self.loss_cls(
                flatten_cls_scores, flatten_labels, flatten_center,
        ...
        if num_pos > 0:
            ...
            pos_decoded_target_preds = distance2bbox(pos_points,
                                                     pos_bbox_targets)
            #no centerness weighted iou loss
